...
const EditExistingScreen = ({ navigation, route }: any) => {
  ...
  const populateFormWithSessionData = (data: any) => {
    console.log('Populating form with session data:', data);

    // Set active child
    if (data.childId?._id || data.childId) {
      const childId = data.childId?._id || data.childId;
    ...
    if (data.categoryId?._id || data.categoryId) {
      ...
      const categoryTitle = data.categoryId?.title || 'Unknown Category';
      setCategory(categoryId);

      // Store original category data for dropdown
      setcategorydata([
        {
          label: categoryTitle,
          value: categoryId,
          ageRange: data.childId?.ageRange || '',
        },
      ]);
    }

    // Set selected audio/content
    if (data.contentId?._id || data.contentId) {
      const contentId = data.contentId?._id || data.contentId;
      setcontentState(contentId);
      setSelectedAudios([contentId]);
    }
    ...
    // Set repeat type and days
    if (data.repeat) {
      const repeatType = data.repeat.type?.toLowerCase();
      if (repeatType === 'once') setRepeat('Once');
      else if (repeatType === 'daily') setRepeat('Daily');
      else if (repeatType === 'weekly') {
        setRepeat('Weekly');
        if (data.repeat.daysOfWeek) {
          setSelectedDays(data.repeat.daysOfWeek);
        }
      }
    }
    ...
    if (data.enableNotifications !== undefined) {
      setNotification(data.enableNotifications);
    }
  };
  ...
  const fetchAudioContent = async (categoryId: any) => {
    ...
    try {
      ...
      if (!apicall_content.error && apicall_content.data?.list) {
        ...
        // Preselect the existing audio if it's in the list
        if (contentState) {
          const existingAudio = audioData.find(
            (audio: any) => audio.id === contentState,
          );
          if (existingAudio) {
            setSelectedAudios([contentState]);
          }
        }
  ...
  const UpdateSchedule = async () => {
    ...
    const payload: any = {
      scheduleContentId: sessionId, // Add schedule ID for update
      childId: activeChild,
      categoryId: category,
    ...
    console.log('Update Schedule API Payload:', payload);

    try {
      // Use update endpoint
      const res = await apiRequest(ApiURL.updateSession, 'POST', payload, true);
      setLoader(false);
      ...
      if (!res?.error) {
        ...
        const errorMsg =
          res?.message || 'Failed to update schedule. Please try again.';
        Alert.alert('Error', errorMsg);
      }
    } catch (error: any) {
      setLoader(false);
      console.log('Update Schedule API Error:', error);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: moderateScale(80) }}>
        <Text style={styles.mainTitle}>
          {languageData?.edit_schedule_title || 'Edit Schedule'}
        </Text>
        ...
        <View style={styles.dropdownContainer}>
          ...
          <CustomDropdown
            data={categorydata}
            placeholder={
              languageData?.select_categories_placeholder || 'Select Categories'
            }
            value={category}
            onChange={(item: any) => {
          ...
          >
            ...
            >
              {selectedAudios.length > 0
                ? languageData?.audio_selected_count?.replace(
                    '{{count}}',
        ...
        {/* UPDATE BUTTON */}
        <CustomButton
          text={languageData?.update_schedule_button || 'Update Schedule'}
